...
from core.extractor import MultiBasicEncoder, Featurewithmsfa
from core.geometry import Combined_Geo_Encoding_Volume
from core.submodule import *
import time
from nets.refinement import GIRN
from nets.PECV import pecv
...
class GIP(nn.Module):
    ...
    def forward(self, image1, image2, iters=22, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            combined_features = [torch.cat([left, right], dim=1) for left, right in zip(features_left, features_right)]
            Propagation-Enhanced Correlation Volume = self.pecv(combined_features)

            Propagation-Enhanced Correlation Volume = self.cost_agg(gwc_volume, Propagation-Enhanced Correlation Volume)

            prob = F.softmax(self.classifier(Propagation-Enhanced Correlation Volume).squeeze(1), dim=1)
        ...
        geo_fn = geo_block(match_left.float(), match_right.float(), Propagation-Enhanced Correlation Volume.float(), radius=self.args.corr_radius, num_levels=self.args.corr_levels)
        ...
        for itr in range(iters):
            ...
            if itr == iters - 1:
                refine_value = self.girn(disp_up, image1, image2)
